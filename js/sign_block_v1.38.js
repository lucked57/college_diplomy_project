...
function HideBlock(block_div_h){
   ...
   function ShowBlock_acc(block_div){ // эта функция осуществляет появление блока
       ...
       $('#'+block_div).fadeIn(400);
       
       if ($(window).width() <= 1206 || $(window).width() >= 1850){
           AddBlur(); // вызывается функция, добавлющая эффект размытия
           
           
           modal_div.fadeIn(400);
       }
       else{
           modal_div.css("background-color", "rgba(0, 0, 0, 0.65)");
           modal_div.fadeIn(400);
...
function HideBlock_acc(block_div_h){
    ...
    if ($(window).width() <= 1206 || $(window).width() >= 1850){
        ...
        $('#'+block_div_h).fadeOut(400);
          modal_div.fadeOut(400);
...
$(function() {
    $('#delete_achivka_for_t, #close_login55').click(function(){
        
        var block_div = 'accept_t_d';
        
     ShowBlock_acc(block_div);
    ...
    $('#renouncement').click(function(){
       ...
       var block_div_h = 'accept_t_d';
        HideBlock_acc(block_div_h);
