...
function AddBlur(){
    ...
    if ($(window).width() <= 1206){ // если ширина экрана устойства меньше 1206px
        $('.cd-header, .blur, .cd-fixed-bg').css("transition", "0.71s"); // скорость css анимации у выбранный классов устанавливается на 0.6 секунд
        $(".cd-header").css("filter", "blur(60px)"); // к классу cd-header добавляется эффект размытия, с помощью фильтра blur
            $(".blur, .cd-fixed-bg").css("filter", "blur(60px)");
            ...
            $(".blur, .cd-fixed-bg").css("filter", "blur("+px+"px)");
                    ...
                    px++;
                    if (px_h < 150){ //чтобы у меню было размытие меньше 12px
                        px_h++;
                    }
                    }, 7);
