...
function ShowMessengeFun(messenge){
    $(".messenge").fadeOut(300);
    
      setTimeout(function() {
                                $(".messenge p").html(messenge);
                            }, 300);
...
$(function() {
    $('#close_messenge5').click(function(){

     
        $('.messenge_href5').animate({ opacity: 'toggle',  top: 'toggle' }, 600);
   ...
   $('#again_change_email_code').click(function(){
         ...
         setTimeout(function() {
          ...
          $("#do_change_email_ajax, #again_change_email_code").on("click", function(){
            ...
            function funcSuccessAdminAdd(data){
                
                Hideloader();
                
                    if(data == "add"){
                      ShowMessengeFun('Данные, которые прошли валидацию успешно добавлены');
                    }
                    else{
                        ShowMessengeFun(data);
                    }
          ...
          $("#do_search, #do_prepod, #do_groupe").on("click", function(){
              
              var errors = false;
              
               if((this.id) == 'do_search'){
                   var type = "students";
               }
              
                if((this.id) == 'do_prepod'){
                   var type = "prepod";
               }
              
                if((this.id) == 'do_groupe'){
                   var type = "gpoupe";
               }
              
              
              var text = $("#text_add_for_admin").val();
              ...
              if(errors == false){
               ...
               $.ajax({
                ...
                dataType: "html",
                beforeSend: Showloader,
                success: funcSuccessAdminAdd
            });
              
              }
              else{
                  ShowMessengeFun(errors);
              }
...
function funcBefore(){
    $(".messenge").fadeOut(300);
       ...
       $(".upsign").animate({ opacity: 'toggle',  top: 'toggle' }, 600);
            ...
            function funcSuccess(data){ // Функция, которая будет вызвана после завершения запроса к серверу, где data данные, присланные сервером 
                Hideloader(); // скрытие анимации
                $(".upsign").fadeOut(300);
          ...
          $("#do_login_ajax, #form_login").on("submit", function(){ // при событии отправки формы с id form_login
             ...
             var login = $("#login_l").val(); // получение содержимого элементов формы
              ...
              if(errors == false){ // если данные введены пользователем корректно, то отправляем запрос к серверу с помощью метода ajax
                    $.ajax({
                  url: "login_ajax_port.php", // адрес, на который отправляется запрос
                  ...
                  success: funcSuccess // Функция, которая будет вызвана после завершения запроса к серверу.
              });
            ...
            function funcSuccessS(data){
                ...
                if(data == "visibale"){
                   ...
                   // $(".messenge").fadeOut(300);
                     ...
                     if(data == "На ваш email уже был отправлен код подтверждения, проверьте спам или нажмите отправить повторно"){
...
function LimitPosts(limit_0){
    if (limit_0 == 0){ // если уменьшать больше некуда, то скрыть кнопку уменьшения, иначе показать
    $("#minus").hide();
...
function funcBeforeClick(){ 
    $(".messenge").fadeOut(300);
            ...
            function funcSuccessClick(data){ // функция, которая будет вызвана после завершения запроса к серверу
                $(document).ready(function(){
                Hideloader_m();
                    ...
                    shitchik = 0;
                              ...
                              for(var id in data){ // добавление к блоку с id contentposts блоков с достижениями
     ...
     $("#plus").on("click", function(){ // увелечение значения переменной limit_0 на 12(limit_0 - кол-во строк, которые будут пропущены в sql запросе)
                    ...
                    $("#minus").on("click", function(){ // уменьшения значения переменной limit_0 на 12(limit_0 - кол-во строк, которые будут пропущены в sql запросе)
          ...
          $("#plus, #minus").on("click", function(){ // запрос к серверу
...
$('#search_query').on("input", function() {
    ...
    if(query.length >= 2 && query_ajax == true){
        //alert(query);
        
          if(query.indexOf('<')!= -1 || query.indexOf('>')!= -1 || query.indexOf("'")!= -1){
              $('.search_result').empty();
                  $('.search_result').append("<li>В вашем запросе содержаться недопустимые символы</li>");
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                if(data == "empty"){ 
                    ...
                    $('.search_result').append("<li>По запросу "+query+" ничего не найдено</li>");
                                  $('.search_result').fadeIn(300);
