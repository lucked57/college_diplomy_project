...
<?php
    ...
    $gropue = $Select->get_select_where_fetch_assoc('teatchers_anketa','login',$this_login);

    $groupe_name = $gropue['Group_name'];
        ...
        if($imageinfo['mime'] != 'image/png' && $imageinfo['mime'] != 'image/jpeg' && $imageinfo['mime'] != 'image/jpg') {
          $errors = 'Неподдерживаемый тип файла min';
            ...
            $select_level = $pr->proverka_input($_POST['select_level']);
             ...
             if(empty($title) || empty($date) || empty($tags) || empty($text) || empty($select_type) || empty($select_category) || empty($groupe_name)){
               ...
               if(filesize($copyfile) > 30000) {
                ...
                if($j == 0)    {
                   // $Insert->insert_values_four("img_post","img_min","img_full","title","text",$file_put,$file_put_full,$title,$text);
                    ...
                    $SQl = new SQL_native();
                    $SQl->query("insert into img_post (img_min, img_full, title, text, data, date, tags, type, category, level, teacher, group_name) values ('$file_put','$file_put_full','$title','$text','$data','$date','$tags','$select_type','$select_category','1','$this_login','$groupe_name')");
            ...
            <div class="row justify-content-around mt-5 pt-5">
                    ...
                    <div class="col-xl-4 col-md-8">
                       <?php if (!empty($errors)): ?>
                         ...
                         <? endif; ?>
                        <form action="/add_post_teacher.php" method="POST" enctype="multipart/form-data"> 
                            <div class="card">
                              ...
                              <div class="card-img-overlay">
                                   <div class="custom-file text-center">
                                       <div class="col  align-self-center">
                                        <input name="image[]" type="file" multiple="multiple" id="customFile" accept="image/png,image/jpeg" class="mt-5 pt-md-5"/>
                                        </div>
                                    </div>
                                </div>
                              <div class="card-body">
                                <div class="form-group row">
                                      ...
                                      <div class="col-sm-9">
                                        <input type="text" name="title" class="form-control b-r" placeholder="Введите название..."  value="<?php echo $title;?>">
                                      </div>
                                </div>
                                <!--<div class="form-group row">
                                ...
                                <div class="form-group row">
                                      ...
                                      <div class="col-sm-9">
                                        <input type="text" name="tags" class="form-control b-r" placeholder="Перечеслите через пробел теги..." value="<?php echo $tags;?>">
                                      </div>
                                </div>
                            ...
                            <div class="form-group mt-3">
                             ...
                             <div class="form-group mt-1">
                                    ...
                                    <textarea class="form-control" name="text" rows="5" placeholder="Описание поста..." value="<?php echo $text;?>"></textarea>
                                  </div>
                                  

                                  
                                 
                                  <select class="custom-select mb-3 b-r" name="select_category">
                                  ...
                                  <select class="custom-select mb-3 b-r" name="select_type">
